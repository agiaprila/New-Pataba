...
from publication.models import Districts, PPPA
from flask import render_template, flash, redirect, url_for, request
from publication.forms import FormPPPA
from flask_login import current_user, login_required

# ------------------------------------  ( PPPA) --------------------------------------------
# pppa (Kota)
@app.route('/publikasi/pppa')
def pppa():
  data = PPPA.query.filter_by(district_id=None).order_by(PPPA.tahun).all()
  return render_template('pppa/pppa.html', data=data)

# pppa (Kecamatan)
@app.route('/publikasi/pppa/<int:district_id>')
def pppa_kec(district_id):
  data = PPPA.query.filter_by(district_id=district_id).order_by(PPPA.tahun).all()
  # id_path = int(str(request.path)[-1])
  district_name = Districts.query.filter_by(id=district_id).first()
  # return render_template('pppa/pppa_kec.html', data=data, district_id=district_id, id_path=id_path, district_name=district_name)
  return render_template('pppa/pppa_kec.html', data=data, district_id=district_id, district_name=district_name)

# edit tabel
@app.route('/publikasi/pppa/add', methods=['GET', 'POST'])
@login_required
def pppa_add():
  if current_user.role == 'admin' or current_user.officer_of_agency == 19:
    form = FormPPPA()
...
#      if current_user.role == 'admin' or current_user.officer_of_district == form.district_id.data:
      rows_to_create = PPPA(tahun=form.tahun.data,
                              u1=form.u1.data,
                              u2=form.u2.data,
                              u3=form.u3.data,
                              u4=form.u4.data,
...
# hapus record
@app.route('/publikasi/pppa/delete/<int:id>')
@login_required
def pppa_delete(id):
  row_to_delete = PPPA.query.filter_by(id=id).first()
  if current_user.role == 'admin' or current_user.officer_of_agency == 19:
